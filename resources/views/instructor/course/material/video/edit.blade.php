	...
	<div class="container">
		...
		<div class="card">
			...
			<div class="card-body">
				...
				<form method="POST" action="/dashboard/instructor/course/update_material/video/{{$course->course_id}}/{{$course->section_id}}/{{$course->material_id}}">
					@csrf
					<div class="row mt-3">
						<div class="col-md-12">
							<label>Judul Materi</label>
							<input value="{{$course->material_title}}" type="text" autocomplete="off" class="form-control" required name="material_title" placeholder="Template Blade Laravel">
						</div>
					</div>
					...
					<div class="row mt-3">
						<div class="col-md-6">
							...
							<input value="{{$course->material_video_url}}" placeholder="https://www.youtube.com/embed/cHnKrLksWGk" type="text" required name="material_video_url" class="form-control" autocomplete="off">
							<small class="text-muted">*Klik kanan pada video youtube dan pilih kode semat</small>
						</div>

						<div class="col-md-2">
							<label>Durasi Video</label>
							<input value="{{$course->duration}}" placeholder="20 Menit" type="number" required name="duration" class="form-control" autocomplete="off">
						</div>

						<div class="col-md-4">
							<label>Overview</label>
							<select class="form-control" name="is_overview" required="">
								<option value="1" <?= $course->is_overview == '1' ? ' selected="selected"' : '' ?> >Ya</option>
								<option value="0" <?= $course->is_overview == '0' ? ' selected="selected"' : '' ?> >Tidak</option>
							</select>
